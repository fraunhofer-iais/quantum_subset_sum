...
output_file = os.path.join(project_path, 'data/financial_documents/parsed_consincome.jsonl')

factor = 100_000_000
tab = Table()
tab.from_excel(file_path, sheetname="ConsIncome")

# rows of the sheet that contain sums, with corresponding solution
sum_indices_excel = {7: [5, 6],
                     9: [7, 8],
...
for i, vector in enumerate(numeric_vectors[:11]):
    ...
    for sum_row, children_rows in sum_indices.items():
        ...
        if sum(children_values) == sum_value:
            ...
            subset_sum_problem = convert_to_subset_sum(numeric_vector, sum_index, children_indices,
                                      name=f'column_{i}_row_{sum_row}',
                                      group='consincome')
            QUBO.from_numbers_and_target_sum(np.array(subset_sum_problem.numbers), np.array(subset_sum_problem.target))
            vector_output.subset_sum_problems.append(subset_sum_problem)
        else:
...
for sum_row in sum_indices.keys():
